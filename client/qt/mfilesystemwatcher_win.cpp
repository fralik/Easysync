...
void ModifiedWindowsFileSystemWatcherEngine::stop()
{
    foreach(ModifiedWindowsFileSystemWatcherEngineThread *thread, threads)
        thread->stop();
}

ModifiedWindowsFileSystemWatcherEngine::ModifiedWindowsFileSystemWatcherEngine()
    : MFileSystemWatcherEngine(false)
{
}
...
QStringList ModifiedWindowsFileSystemWatcherEngine::addPaths(const QStringList &paths,
                                                       QStringList *files,
                                                       QStringList *directories)
{
    ...
    while (it.hasNext()) {
        ...
        ModifiedWindowsFileSystemWatcherEngine::PathInfo pathInfo;
        ...
        // Look for a thread
        ModifiedWindowsFileSystemWatcherEngineThread *thread = 0;
        ModifiedWindowsFileSystemWatcherEngine::Handle handle;
        QList<ModifiedWindowsFileSystemWatcherEngineThread *>::const_iterator jt, end;
        ...
        for(jt = threads.constBegin(); jt != end; ++jt) {
            ...
            if (handle.handle != INVALID_HANDLE_VALUE && handle.flags == flags) {
                ...
                QHash<QString, ModifiedWindowsFileSystemWatcherEngine::PathInfo> &h
        ...
        if (handle.handle == INVALID_HANDLE_VALUE || handle.flags != flags) {
            ...
            handle.handle = FindFirstChangeNotification((wchar_t*) QDir::toNativeSeparators(effectiveAbsolutePath).utf16(), true, flags);
            handle.flags = flags;
            ...
            if (!found) {
                ModifiedWindowsFileSystemWatcherEngineThread *thread = new ModifiedWindowsFileSystemWatcherEngineThread();
...
QStringList ModifiedWindowsFileSystemWatcherEngine::removePaths(const QStringList &paths,
                                                          QStringList *files,
                                                          QStringList *directories)
{
    ...
    while (it.hasNext()) {
        ...
        QList<ModifiedWindowsFileSystemWatcherEngineThread *>::iterator jt, end;
        end = threads.end();
        for(jt = threads.begin(); jt!= end; ++jt) {
            ModifiedWindowsFileSystemWatcherEngineThread *thread = *jt;
            if (*jt == 0)
                continue;

            QMutexLocker locker(&(thread->mutex));

            ModifiedWindowsFileSystemWatcherEngine::Handle handle = thread->handleForDir.value(absolutePath);
            if (handle.handle == INVALID_HANDLE_VALUE) {
...
///////////
// ModifiedWindowsFileSystemWatcherEngineThread
///////////

ModifiedWindowsFileSystemWatcherEngineThread::ModifiedWindowsFileSystemWatcherEngineThread()
        : msg(0)
{
...
void ModifiedWindowsFileSystemWatcherEngineThread::run()
{
    ...
    forever {
        ...
        do {
            if (r == WAIT_OBJECT_0) {
                ...
                if (m != '@')  {
                    qDebug("ModifiedWindowsFileSystemWatcherEngine: unknown message '%c' send to thread", char(m));
                }
                break;
                ...
                if (handles.contains(handle)) {
                    ...
                    if (!FindNextChangeNotification(handle)) {
                        qErrnoWarning("ModifiedFileSystemWatcher: FindNextChangeNotification failed!!");
                    }

                    QHash<QString, ModifiedWindowsFileSystemWatcherEngine::PathInfo> &h = pathInfoForHandle[handle];
                    QMutableHashIterator<QString, ModifiedWindowsFileSystemWatcherEngine::PathInfo> it(h);
                    while (it.hasNext()) {
                        QHash<QString, ModifiedWindowsFileSystemWatcherEngine::PathInfo>::iterator x = it.next();
                ...
                // qErrnoWarning("ModifiedFileSystemWatcher: error while waiting for change notification");
...
void ModifiedWindowsFileSystemWatcherEngineThread::stop()
{
    msg = 'q';
    SetEvent(handles.at(0));
}

void ModifiedWindowsFileSystemWatcherEngineThread::wakeup()
{
    msg = '@';
    SetEvent(handles.at(0));
}
