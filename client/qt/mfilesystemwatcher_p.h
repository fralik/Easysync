...
#ifndef MFILESYSTEMWATCHER_P_H
#define MFILESYSTEMWATCHER_P_H
...
class MFileSystemWatcherEngine : public QThread
{
    Q_OBJECT

protected:
    inline MFileSystemWatcherEngine(bool move = true)
    {
        if (move)
            moveToThread(this);
    }
...
class MFileSystemWatcherPrivate : public QObjectPrivate
{
    Q_DECLARE_PUBLIC(MFileSystemWatcher)

    static MFileSystemWatcherEngine *createNativeEngine();

public:
    MFileSystemWatcherPrivate();
    void init();
    void initPollerEngine();
    void initForcedEngine(const QString &);

    MFileSystemWatcherEngine *native, *poller, *forced;
